...
async function playMelody() {
    ...
    try {
        do {
            ...
            for (let i = 0; i < notes.length; i++) {
                ...
                if (item.isChord) {
                    ...
                    visualizer.innerHTML = `<span style="font-size: 48px;">🎹</span><br><span style="font-size: 18px;">${noteNames}</span>`;
                } else {
                    const frequency = midiToFrequency(item.note);
                    playNote(frequency, duration, item.velocity);
                    visualizer.innerHTML = `<span style="font-size: 48px;">🎵</span><br><span style="font-size: 24px;">${item.noteText}</span>`;
                }
                
                const loopText = loopEnabled ? ` [Цикл ${loopCount}]` : '';
                timeline.textContent = `Элемент ${i + 1} из ${notes.length}${loopText}`;
            ...
            if (loopEnabled && !shouldStop) {
                ...
                visualizer.innerHTML = '<span style="font-size: 48px;">🔄</span><br><span style="font-size: 14px;">Повтор...</span>';
        ...
        if (!shouldStop) {
            showToast(`✅ Воспроизведение завершено (${loopEnabled ? loopCount + ' циклов' : '1 раз'})`);
        }
        ...
        visualizer.innerHTML = '<span>🎵</span>';
...
function stopMelody() {
    ...
    setTimeout(() => {
        if (isPlaying) {
            console.warn('⚠️ Принудительный сброс isPlaying');
            isPlaying = false;
        }
    }, 100);
...
function loadPreset(type) {
    ...
    const presets = {
        ...
        'melody': [
            ...
            {isChord: false, note: 64, noteText: 'E4 (Ми)', duration: 96, durationText: '♩ Четвертн��я (1/4)', velocity: 95},
            {isChord: false, note: 67, noteText: 'G4 (Соль)', duration: 96, durationText: '♩ Четвертная (1/4)', velocity: 100},
